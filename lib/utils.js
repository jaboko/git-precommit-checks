...
const EXTERNAL_JS_CONF = 'git-precommit-checks.config.js'
const EXTERNAL_JSON_CONF = 'git-precommit-checks.json'
...
var rootPath;
if (!rootPath) {
    rootPath = path.join(path.resolve(execSync('git rev-parse --git-dir').toString()), '..');
}
...
async function getStagedContent(file, hookTitle) {
  const { stdout } = await exec(gitShowStaged(path.join(rootPath, file)), hookTitle)
...
// Load configuration from `git-precommit-checks.config.js`.
// If none can be found, fallback on JSON configuration.
async function loadConfiguration() {
  const path = await findUp(EXTERNAL_JS_CONF)

  return path
    ? { config: require(path), json: false }
    : { config: await loadJSONConfiguration(), json: true }
}

// Load configuration from closest `git-precommit-checks.json`
async function loadJSONConfiguration() {
  const loadJsonFile = require('load-json-file')
  const path = await findUp(EXTERNAL_JSON_CONF)
